from contracts.lido import Lido
from contracts.vault import Vault
from contracts.wrapper_pool import WrapperPool
from scripts.utils.print_users_pool import print_users

wrapper = WrapperPool(Vault(Lido(), liability_limit_steth=1_000_000, reserve_ratio=0.2))


def main():
    wrapper.stake_eth("user 1", 100_000)
    wrapper.stake_eth("user 2", 50_000)

    print_users(wrapper, "Initial state")

    wrapper.vault.increase_liability_steth(50_000)
    print_users(wrapper, "Vault gets extra debt")

    wrapper.rebalance_total_shared_liability_wsteth(wrapper.get_total_shared_liability_wsteth() // 2)
    print_users(wrapper, "Half of the shared debt rebalanced")

    wrapper.rebalance_total_shared_liability_wsteth(wrapper.get_total_shared_liability_wsteth())
    print_users(wrapper, "Whole shared debt rebalanced")


if __name__ == "__main__":
    main()
